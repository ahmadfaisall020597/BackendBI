...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $kalangan = PesertaPendaftaran::select(
            'kalangan_id',
            'nama_kalangan',
        ...
        $iklan = PesertaPendaftaran::join(
            'iklans',
            'peserta_pendaftarans.iklan_id',
            '=',
            'iklans.id'
        )
            ->select(
                ...
                DB::raw('SUM(peserta_pendaftarans.biaya_pendaftaran) -
                 (COUNT(peserta_pendaftarans.id) * iklans.biaya) as keuntungan')
            )
            ->groupBy('iklans.id', 'iklans.nama_iklan', 'iklans.biaya')
            ->orderByDesc('total_peserta')
            ->get();

        $webinar = PesertaPendaftaran::select(
            'webinar_id',
            'nama_webinar',
            DB::raw('COUNT(*) as total_peserta')
        )
            ->groupBy('webinar_id', 'nama_webinar')
            ->orderByDesc('total_peserta')
            ->limit(10)
            ->get();

        $kalanganTerbanyak = PesertaPendaftaran::select(
            'kalangan_id',
            'nama_kalangan',
            DB::raw('COUNT(*) as total_peserta')
        )
        ...
        return response()->json([
            ...
            'iklan' => $iklan,
            'webinar' => $webinar,
            'kalangan_terbanyak' => $kalanganTerbanyak
        ]);
    }
}
